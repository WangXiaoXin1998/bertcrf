...
class DataPrecessForSentence(Dataset):
    ...
    def get_input(self, file, is_sentence=False):
        ...
        if isinstance(file, list):
            ...
            if n_nums:
                df = pd.read_csv(
                    file, engine='python', encoding=csv_encoding, error_bad_lines=False, nrows=n_nums)
            else:
        ...
        if csv_rows[-1] in df.columns:
            ...
            data['labels'] = labels
        print('输入例子')
        print(sentences[0] if isinstance(
            sentences[0], str) else sentences[0][0])
    ...
    def my_bert_tokenizer(self, sentences, padding, truncation, return_tensors):
        ...
        sentences_list = [['[CLS]'] + [i for i in sen] + ['[SEP]']
                          for sen in sentences]
        input_ids = [self.bert_tokenizer.convert_tokens_to_ids(
            sen) for sen in sentences_list]
        # print(input_ids)
        # print(f"max_len 为{max_len}")
        attention_mask = [
            [1, ] * len(i) + [0, ] * (max_len - len(i)) for i in input_ids]
        ...
        data_dict = {k: torch.Tensor(v).type(torch.long)
...
def my_metrics(decode_labels, decode_preds, id2label=None):
    ...
    for type_, count in origin_counter.items():
        ...
        class_info[type_] = {"acc": round(precision, 4), 'recall': round(
            recall, 4), 'f1': round(f1, 4), 'num': count}
    origin = len(origins)
    found = len(founds)
    right = len(rights)
    recall, precision, f1 = compute(origin, found, right)
...
def data_t(model, dataloader, tokenizer):
    ...
    with torch.no_grad():
        for tokened_data_dict in tqdm(dataloader):
            ...
            tokened_data_dict = {k: v.to(device)
            ...
            tags = model.crf.decode(
                logits, tokened_data_dict['attention_mask'])
...
class Inferenve:
    ...
    def get_ret(self, sentences):
        ...
        #     print(i)
        batch_time, total_time, accuracy, all_labels, all_pred = data_t(
            self.model, test_loader, self.bert_tokenizer)
        # decoded_preds = ner_decode(all_pred)
        all_pred = [[j.item() for j in i.cpu()] for i in all_pred]
        label_entities = [get_entities(
            i, self.id2label_dict, 'bio') for i in all_pred]
...
tts_parser.add_argument('text', type=str, required=True,
                        help="想要转语音的文字", default='高勇：男，中国国籍')
